...
/// 4. changed functionality such that plot can be called many times
///    and for the same name of the plot, the same temporary
///    file is used again
///    by P. Speckmayer
///
/// Requirements:
...
class Gnuplot
{
    ...
    typedef std::map<std::string,std::string> TmpFileMap;
    TmpFileMap tmpfile_map; 
	...
	// ---------------------------------------------------                          
    std::string    create_tmpfile(std::string title, std::ofstream &tmp);  
...
Gnuplot& Gnuplot::plot_x(const X& x, const std::string &title)
{
    ...
    std::ofstream tmp;
    std::string name = create_tmpfile(title,tmp);
    if (name == "")
        return *this;
...
//
std::string Gnuplot::create_tmpfile(std::string title, std::ofstream &tmp)
{
    TmpFileMap::iterator it = tmpfile_map.find (title);
    if (it != tmpfile_map.end ())
    {
    ...
    tmpfile_map.insert (std::make_pair(title, name));
    return name;
}
